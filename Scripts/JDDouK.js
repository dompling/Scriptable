...
class Widget extends DmYY {
	...
	graphLow = 214;
	graphHeight = 125;
	spaceBetweenDays = 100; // 间距

	accentColor1 = new Color("#33cc33", 1);
	accentColor2 = new Color("#6f7374", 0.7);
	...
	drawImage = async () => {
		...
		// Vertical Line
		const axisX = new Point(2, 20);
		const axisY = new Point(2, this.graphLow);
		this.drawLine(axisX, axisY, this.vertLineWeight * 8, this.accentColor1);

		const axisW = new Point(this.widgetWidth, this.graphLow);
		this.drawLine(axisY, axisW, this.vertLineWeight * 8, this.accentColor1);
		...
		for (let i = 0, j = highestIndex; i < rangeKeys.length; i++, j--) {
			...
			this.drawLine(point1, point2, this.vertLineWeight * 60, this.accentColor2);
	...
	renderJDHeader = async (header) => {
		...
		const textItem = headerMore.addText(`${this.JDCookie.userName}:${this.beanCount}🐶`);
