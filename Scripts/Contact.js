...
class Widget extends DmYY {
		...
		try {
			...
			if (!this.userName) {
				...
				this.dataSource = data.find(item => {
					return (item.familyName === this.userName ||
	...
	setAvatar = (w) => {
		...
		if (this.dataSource.image) {
			...
			textFormat.size = 42;
			this.provideText(this.userName.substr(0, 1) || "", stackAvatar, textFormat);
		}
		stackLeft.addSpacer();
	...
	Run() {
		if (config.runsInApp) {
			this.registerAction("右侧透明", async () => {
				await this.setAlertInput("右侧透明", "若不需要右侧背景设置透明度 0 即可", { rightOpacity: "透明度 0~1" });
			});
			this.registerAction("基础设置", this.setWidgetConfig);
		}
		const light = new Color(this.settings.lightColor, parseInt(this.settings.rightOpacity || 1));
		const dark = new Color(this.settings.darkColor, parseInt(this.settings.rightOpacity || 1));
