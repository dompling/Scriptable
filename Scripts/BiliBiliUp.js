...
class Widget extends DmYY {
	...
	getData = async () => {
		let data = await this.$request.get("https://t.me/s/" + this.inputValue, "STRING");
		data = data.match(/tgme_channel_info_header">(.|\n)+tgme_channel_download_telegram"/)[0];
		this.dataSource.logo = data.match(/https.+jpg/)[0];
		this.dataSource.title = data.match(/header_title"><span dir="auto">(.+)<\/span>/)[1];
		let entities = this.dataSource.title.match(/&#\d{2,3};/g);
		if (entities) {
			for (let k in entities) {
				let rExp = new RegExp(entities[k], "g");
				this.dataSource.title = this.dataSource.title.replace(rExp, this.entityToString(entities[k]));
			}
		}
	...
	setPathStack = (stack) => {
		...
		const titleItem = this.provideText(`@${this.inputValue}`, stack, textFormatNumber);
		titleItem.lineLimit = 1;
	};
